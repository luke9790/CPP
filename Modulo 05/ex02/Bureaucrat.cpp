...
void Bureaucrat::executeForm(AForm const &from) 
{
	...
	{
		from.execute(*this);
		std::cout << Name << " executed the form requested by " << from.getName( ) << std::endl;
	} 
	catch (std::exception& e) 
	{
		std::cout << Name << " tried to execute the form requested by " << from.getName( ) << ", but " << e.what( ) << std::endl;
	}
}