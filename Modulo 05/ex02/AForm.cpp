...
void AForm::beSigned(const Bureaucrat& raw) 
{
    try 
    {
        if (raw.getGrade( ) <= gradeToSign) {
            IsSigned = true;
            std::cout << raw.getName( ) << " has successfully signed the form requested by " << this->Name << std::endl;
        }
        else
            throw(GradeTooLowException( ));
    }
    catch(const std::exception& e) {
        std::cout << raw.getName( ) << " is unable to sign the form requested by " << this->Name << "\n";
...
std::ostream& operator<<(std::ostream& os, const AForm& raw) 
{
    ...
    os << "signed: ";
    raw.getIsSigned( ) != 0 ? os << "yes" : os << "no";
    os << std::endl;
